...
namespace Delivery.Application.Tests.Services.OrderService
{
    public class GetByIdOrderServiceTests
    {
        ...
        {
            ulong orderId = 2;
            var order = new Order() { AvailableTo = "Ulitsa A.Samadov, Dom 24", AvailableFrom = "Restourant Forel", DriverId = 4 };
            var orderResponse = new CreateOrderResponse() { AvailableTo = "Ulitsa A.Samadov, Dom 24", AvailableFrom = "Restourant Forel", DriverId = 4 };
            ...
            Assert.That(4, Is.EqualTo(result.DriverId));
        }
    }
}
